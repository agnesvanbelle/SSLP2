...
#get the reorderings recursively
...
def get_sentence_and_indexes_rec_helper(sentence_tree, rlist, slist, index=0, pos=0):
  ...
  if not isinstance(sentence_tree, Tree) or  len(sentence_tree) == 1: # leaf
    rlist[index] = pos   
    ...
    if (sentence_tree.node[-len(inv_suffix):] == inv_suffix): #inversed rule
      temp = right
      right = left
      left = temp
      temp = addright
...
#sentence, indexes, x = get_sentence_and_indexes(tree_string)
...
if __name__ == '__main__':
  tree_string = '(S (NP (N man)) (VP< (V bites) (NP (N dog))))'
  #tree_string = '(A (B< a b) (C c d))'
